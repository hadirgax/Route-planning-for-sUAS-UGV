...
```{python}
UGV_route, mission_locations, routes, solution, routing, manager = UGV.main(_locations=_locations)
```
...
# Finding the drone routes by local search heutistics

I modify the code of `MCsim_3veh_2cluster_3clustdist_bounds_one.py` to help understanding and usage of the code. There is no guide on how to use the code but there was a outside loop (now in the main function) and a main funtion (now on the optimize function), I think the usage was to call the loop and the the main.
```{r}
import importlib; importlib.reload(MCsim)
mission_locations, interpolated_UGV_stops = MCsim.main(nb_exp=1, list_locations=[mission_locations])
MCsim.optimize(mission_locations, interpolated_UGV_stops)
```
